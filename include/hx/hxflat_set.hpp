...
/// `insert` returns a pointer to the existing element. When set, duplicate keys
/// are always inserted. `compare_t` defaults to `hxkey_three_way_t`, a callable
/// with signature returning a value less than, equal to, or greater than zero,
/// and `traits` defaults to `hxtrait_three_way`. Passing `hxkey_less_t` and
/// clearing `hxtrait_three_way` in `traits` selects a strict weak order
/// `compare_t` instead, with signature `bool(const key_t_&, const key_t_&)`
/// returning true when the first argument is ordered before the second.
///
