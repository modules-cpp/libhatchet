// SPDX-FileCopyrightText: © 2017-2026 Adrian Johnston.
// SPDX-License-Identifier: MIT
// This file is licensed under the MIT license found in the LICENSE.md file.

#include <hx/hxhash_table_nodes.hpp>
#include <hx/hxhash_table.hpp>
#if HX_CPLUSPLUS >= 202302L
#include <hx/hxexpected.hpp>
#endif // HX_CPLUSPLUS >= 202302L
#include <hx/hxtest.hpp>

#include "./hxtest_util.hpp"

HX_NS_USE

hxattr_noinline static void hxtest_gdb_break_hxhash_table(void) { }
hxattr_noinline static void hxtest_gdb_break_hxhash_table_edge_cases(void) { }

namespace {
using hxhash_table_test_f = hxtest_object_fixture;

class hxtest_integer : public hxhash_table_node_integer<int32_t> {
public:
	hxtest_integer(int32_t k, int32_t v) :
		hxhash_table_node_integer(k), value(v) { }
	hxtest_object value;
};

class hxtest_object_node : public hxhash_table_node_integer<hxtest_object> {
public:
	hxtest_object_node(const hxtest_object& k, const hxtest_object& v) :
		hxhash_table_node_integer(k), value(v) { }
	hxtest_object value;
};

class hxtest_string : public hxhash_table_node_string<hxslab_allocator_stack_0> {
public:
	hxtest_string(const char* k, int32_t v) :
		hxhash_table_node_string(k), value(v) { }
	hxtest_object value;
};

class hxtest_string_literal : public hxhash_table_node_string_literal {
public:
	hxtest_string_literal(const char* k) : hxhash_table_node_string_literal(k) { }
};

struct hxtest_set_node_t : hxhash_table_set_node<int32_t> {
	explicit hxtest_set_node_t(int32_t k) : hxhash_table_set_node<int32_t>(k) { }
};
} // namespace

TEST_F(hxhash_table_test_f, null) {
	{
		using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
		table_t table;
		EXPECT_EQ(table.size(), 0);
		const table_t& const_table = table;
		EXPECT_EQ(table.begin(), table.end());
		EXPECT_EQ(table.cbegin(), table.cend());
		EXPECT_EQ(const_table.begin(), const_table.cend());
		EXPECT_EQ(table.begin(), table.end());
		EXPECT_EQ(table.cbegin(), table.cend());
		EXPECT_EQ(const_table.begin(), const_table.cend());
		table.clear();
		table.release_all();
		EXPECT_EQ(table.load_factor(), 0.0f);
	}
	{
		using table_dynamic_t = hxhash_table<hxtest_integer, hxdefault_delete, hxallocator_dynamic_capacity, 0>;
		const table_dynamic_t dynamic_table;
		EXPECT_EQ(dynamic_table.bucket_count(), 0);
		EXPECT_EQ(dynamic_table.load_factor(), 0.0f);
	}
	EXPECT_TRUE(check_no_stats());
}

TEST_F(hxhash_table_test_f, single) {
	static const int k = 77;
	{
		using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
		table_t table;
		const table_t& const_table = table;
		hxtest_integer* node = hxnew<hxtest_integer>(k, 0);
		EXPECT_EQ(&*table.insert(hxptr<hxtest_integer>(node)), node);
		EXPECT_NE(table.begin(), table.end());
		EXPECT_NE(table.cbegin(), table.cend());
		EXPECT_EQ(++table.begin(), table.end());
		EXPECT_EQ(++table.cbegin(), table.cend());
		EXPECT_EQ(table.size(), 1);
		EXPECT_EQ(table.count(k), 1);
		hxtest_integer* dup = hxnew<hxtest_integer>(k, 1);
		EXPECT_EQ(&*table.insert(hxptr<hxtest_integer>(dup)), node);
		const table_t::iterator found = table.find(k);
		EXPECT_EQ(&*found, node);
		EXPECT_EQ(table.find(k, found), table.end());
		const table_t::const_iterator const_found = const_table.find(k);
		EXPECT_EQ(&*const_found, node);
		EXPECT_EQ(const_table.find(k, const_found), const_table.end());
		{
			hxptr<hxtest_integer> extracted = table.extract(k);
			EXPECT_EQ(extracted.get(), node);
			EXPECT_EQ(table.extract(k), hxnil);
			EXPECT_EQ(&*table.insert(hxmove(extracted)), node);
		}
		table.release_all();
		EXPECT_EQ(table.find(k), table.end());
		EXPECT_EQ(table.size(), 0);
		EXPECT_EQ(table.count(k), 0);
		hxtest_integer* new_node = hxnew<hxtest_integer>(k, 2);
		EXPECT_EQ(&*table.insert(hxptr<hxtest_integer>(new_node)), new_node);
		EXPECT_NE(new_node->value.value(), node->value.value());
		EXPECT_EQ(table.size(), 1);
		hxdelete(node);
	}
	EXPECT_TRUE(check_stats(3, 0, 3, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, raw_pointer_insert_duplicate_invokes_deleter) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
	{
		table_t table;
		hxtest_integer* node = hxnew<hxtest_integer>(55, 55);
		// Shares 55's bucket at table_size_bits 4.
		hxtest_integer* const colliding = hxnew<hxtest_integer>(2, 2);
		EXPECT_EQ(&*table.insert(node), node);
		EXPECT_EQ(&*table.insert(colliding), colliding);
		EXPECT_EQ(table.size(), 2);
		EXPECT_EQ(table.count(55), 1);
		EXPECT_EQ(table.count(2), 1);
		hxtest_integer* dup = hxnew<hxtest_integer>(55, 55);
		EXPECT_EQ(&*table.insert(dup), node);
		EXPECT_EQ(table.size(), 2);
		EXPECT_EQ(&*table.find(55), node);
	}
	EXPECT_TRUE(check_stats(3, 0, 3, 0, 0, 0, 0, 0, 0, 0));
	{
		using no_delete_table_t = hxhash_table<hxtest_integer, hxdo_not_delete, 4, 0>;
		no_delete_table_t table;
		hxtest_integer* node = hxnew<hxtest_integer>(55, 55);
		// Shares 55's bucket at table_size_bits 4.
		hxtest_integer* const colliding = hxnew<hxtest_integer>(2, 2);
		EXPECT_EQ(&*table.insert(node), node);
		EXPECT_EQ(&*table.insert(colliding), colliding);
		hxtest_integer* dup = hxnew<hxtest_integer>(55, 55);
		EXPECT_EQ(&*table.insert(dup), node);
		EXPECT_EQ(table.size(), 2);
		hxdelete(dup);
		table.release_all();
		hxdelete(node);
		hxdelete(colliding);
	}
	{
		using single_bucket_table_t = hxhash_table<hxtest_integer, hxdo_not_delete, 1, 0>;
		single_bucket_table_t table;
		hxtest_integer* original = hxnew<hxtest_integer>(0, 0);
		hxtest_integer* nonmatching = hxnew<hxtest_integer>(2, 2);
		EXPECT_EQ(&*table.insert(original), original);
		EXPECT_EQ(&*table.insert(nonmatching), nonmatching);
		hxtest_integer* dup = hxnew<hxtest_integer>(0, 0);
		EXPECT_EQ(&*table.insert(dup), original);
		EXPECT_EQ(table.size(), 2);
		EXPECT_EQ(&*table.find(0), original);
		hxdelete(dup);
		table.release_all();
		hxdelete(original);
		hxdelete(nonmatching);
	}
	EXPECT_TRUE(check_stats(9, 0, 6, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, map_node_usage) {
	using map_node_t = hxhash_table_map_node<int32_t, hxtest_object>;
	using table_t = hxhash_table<map_node_t, hxdefault_delete, 4, 0>;
	{
		table_t table;
		map_node_t* n10 = hxnew<map_node_t>(10);
		EXPECT_EQ(&*table.insert(hxptr<map_node_t>(n10)), n10);
		EXPECT_EQ(&*table.find(10), n10);
		EXPECT_EQ(n10->hash_key(), 10);
		n10->value().value() = 123;
		map_node_t* manual = hxnew<map_node_t>(20);
		manual->value().value() = 321;
		EXPECT_EQ(&*table.insert(hxptr<map_node_t>(manual)), manual);
		EXPECT_EQ(table.size(), 2);
		EXPECT_EQ(table.count(10), 1);
		EXPECT_EQ(table.count(20), 1);
		const table_t& const_table = table;
		const table_t::const_iterator const_lookup = const_table.find(10);
		EXPECT_NE(const_lookup, const_table.end());
		if(const_lookup != const_table.end()) {
			EXPECT_EQ(const_lookup->value().value(), 123);
		}
		const table_t::iterator manual_lookup = table.find(20);
		EXPECT_NE(manual_lookup, table.end());
		if(manual_lookup != table.end()) {
			EXPECT_EQ(manual_lookup->value().value(), 321);
		}
		EXPECT_EQ(table.size(), 2);
	}
	EXPECT_TRUE(check_stats(2, 2, 0, 0, 0, 0, 0, 0, 0, 0));
}

#if HX_CPLUSPLUS >= 202302L
TEST(hxhash_table_test, expected_lookup) {
	using node_t = hxhash_table_map_node<int, int>;
	using table_t = hxhash_table<node_t, hxdefault_delete, 4, 0>;
	table_t table;
	table.try_emplace(1, 1, 10);
	table.try_emplace(2, 2, 20);
	EXPECT_TRUE(table.has_value(1));
	EXPECT_FALSE(table.has_value(3));
	bool called = false;
	EXPECT_EQ(table.and_then(2, [&called](node_t& node) {
		called = true;
		node.value() = 21;
		return hxexpected<int>(false, node.value());
	}), 21);
	EXPECT_TRUE(called);
	called = false;
	EXPECT_FALSE((bool)table.and_then(3, [&called](node_t& node) {
		// GCOVR_EXCL_START
		called = true;
		return hxexpected<int>(false, node.value());
		// GCOVR_EXCL_STOP
	}));
	EXPECT_FALSE(called);
	EXPECT_EQ(table.or_else(2, [&called, &table]() {
		// GCOVR_EXCL_START
		called = true;
		return table.end();
		// GCOVR_EXCL_STOP
	})->value(), 21);
	EXPECT_FALSE(called);
	EXPECT_EQ(table.or_else(3, [&called, &table]() {
		called = true;
		return table.find(1);
	})->value(), 10);
	EXPECT_TRUE(called);
	const table_t& const_table = table;
	called = false;
	EXPECT_EQ(const_table.and_then(2, [&called](const node_t& node) {
		called = true;
		return hxexpected<int>(false, node.value() + 1);
	}), 22);
	EXPECT_TRUE(called);
	called = false;
	EXPECT_FALSE((bool)const_table.and_then(3, [&called](const node_t& node) {
		// GCOVR_EXCL_START
		called = true;
		return hxexpected<int>(false, node.value());
		// GCOVR_EXCL_STOP
	}));
	EXPECT_FALSE(called);
	EXPECT_EQ(hxmove(table).and_then(2, [](node_t&& node) {
		return hxexpected<int>(false, node.value());
	}), 21);
	EXPECT_EQ(hxmove(const_table).and_then(2, [](const node_t&& node) {
		return hxexpected<int>(false, node.value());
	}), 21);
	EXPECT_EQ(const_table.or_else(2, [&called, &const_table]() {
		// GCOVR_EXCL_START
		called = true;
		return const_table.end();
		// GCOVR_EXCL_STOP
	})->value(), 21);
	EXPECT_FALSE(called);
	EXPECT_EQ(const_table.or_else(3, [&called, &const_table]() {
		called = true;
		return const_table.find(1);
	})->value(), 10);
	EXPECT_TRUE(called);
	const node_t fallback(9, 90);
	EXPECT_EQ(table.value_or(2, &fallback)->value(), 21);
	EXPECT_EQ(table.value_or(3, &fallback)->value(), 90);
	EXPECT_EQ(const_table.value_or(2, &fallback)->value(), 21);
	EXPECT_EQ(const_table.value_or(3, &fallback)->value(), 90);
	const auto iterator_and_then = [](const node_t& node) {
		return hxexpected<long>(false, node.value() + 2);
	};
	static_assert(hxis_same<decltype(table.and_then(
		table.find(2), iterator_and_then)), hxexpected<long>>());
	EXPECT_EQ(table.and_then(table.find(2), iterator_and_then).value(), 23);
	EXPECT_FALSE((bool)table.and_then(table.end(), iterator_and_then));
	const auto const_and_then = [](const node_t& node) {
		return hxexpected<long>(false, node.value() + 3);
	};
	EXPECT_EQ(const_table.and_then(
		const_table.find(2), const_and_then).value(), 24);
	EXPECT_FALSE((bool)const_table.and_then(
		const_table.end(), const_and_then));
	const auto iterator_or_else = [&table]() { return table.find(1); };
	static_assert(hxis_same<decltype(table.or_else(
		2, iterator_or_else)), table_t::iterator>());
	static_assert(hxis_same<decltype(table.or_else(
		table.find(2), iterator_or_else)), table_t::iterator>());
	EXPECT_EQ(table.or_else(
		table.find(2), iterator_or_else)->value(), 21);
	EXPECT_EQ(table.or_else(table.end(), iterator_or_else)->value(), 10);
	const auto const_or_else = [&const_table]() { return const_table.find(1); };
	EXPECT_EQ(const_table.or_else(
		const_table.find(2), const_or_else)->value(), 21);
	EXPECT_EQ(const_table.or_else(
		const_table.end(), const_or_else)->value(), 10);
	EXPECT_EQ(table.value_or(table.find(2), &fallback)->value(), 21);
	EXPECT_EQ(table.value_or(table.end(), &fallback)->value(), 90);
	static_assert(hxis_same<decltype(table.value_or(
		table.find(2), &fallback)), table_t::iterator>());
	EXPECT_EQ(const_table.value_or(
		const_table.find(2), &fallback)->value(), 21);
	EXPECT_EQ(const_table.value_or(
		const_table.end(), &fallback)->value(), 90);
}
#endif // HX_CPLUSPLUS >= 202302L

TEST_F(hxhash_table_test_f, multiple) {
	static const int size_i = 78;
	static const unsigned int size_u = 78u;
	{
		using table_t = hxhash_table<hxtest_object_node, hxdefault_delete, hxallocator_dynamic_capacity, hxtrait_multi>;
		table_t table;
		const table_t& const_table = table;
		table.set_size_bits(5);
		for(int i = 0; i < size_i; ++i) {
			hxtest_object_node* n = hxnew<hxtest_object_node>(hxtest_object(i), hxtest_object(i));
			table.insert(hxptr<hxtest_object_node>(n));
			EXPECT_EQ(n->value.value(), i);
			EXPECT_EQ(n->hash_key().value(), i);
		}
		int id_histogram[size_u] = {};
		EXPECT_EQ(table.size(), size_u);
		table_t::iterator it = table.begin();
		table_t::iterator cit = table.begin();
		for(int i = 0; i < size_i; ++i) {
			const table_t::iterator ti = table.find(hxtest_object(i));
			EXPECT_EQ(ti->value.value(), i);
			EXPECT_EQ(table.find(hxtest_object(i), ti), table.end());
			EXPECT_NE(it, table.end());
			EXPECT_NE(cit, table.cend());
			EXPECT_EQ(it, cit);
			EXPECT_LT(static_cast<unsigned int>(it->value.value()), size_u);
			id_histogram[it->value.value()]++;
			EXPECT_LT(static_cast<unsigned int>(cit->value.value()), size_u);
			id_histogram[cit->value.value()]++;
			++cit;
			it++;
		}
		EXPECT_EQ(table.end(), it);
		EXPECT_EQ(table.cend(), cit);
		for(int i = 0; i < size_i; ++i) {
			EXPECT_EQ(id_histogram[i], 2);
		}
		for(int i = 0; i < size_i; ++i) {
			hxtest_object_node* ti = hxnew<hxtest_object_node>(
				hxtest_object(i), hxtest_object(i+size_i));
			EXPECT_EQ(ti->value.value(), i+size_i);
			table.insert(hxptr<hxtest_object_node>(ti));
		}
		int key_histogram[size_u] = {};
		EXPECT_EQ(table.size(), size_u * 2u);
		it = table.begin();
		cit = table.begin();
		for(int i = 0; i < size_i; ++i) {
			const table_t::iterator ti = table.find(hxtest_object(i));
			EXPECT_EQ(ti->hash_key().value(), i);
			const table_t::const_iterator ti2 = const_table.find(hxtest_object(i), ti);
			EXPECT_EQ(ti2->hash_key().value(), i);
			EXPECT_EQ(table.find(hxtest_object(i), ti2), table.end());
			EXPECT_EQ(table.count(hxtest_object(i)), 2);
			EXPECT_LT(static_cast<unsigned int>(it->hash_key().value()), size_u);
			key_histogram[it->hash_key().value()]++;
			++it;
			EXPECT_LT(static_cast<unsigned int>(it->hash_key().value()), size_u);
			key_histogram[it->hash_key().value()]++;
			it++;
			EXPECT_LT(static_cast<unsigned int>(cit->hash_key().value()), size_u);
			key_histogram[cit->hash_key().value()]++;
			++cit;
			EXPECT_LT(static_cast<unsigned int>(cit->hash_key().value()), size_u);
			key_histogram[cit->hash_key().value()]++;
			cit++;
		}
		EXPECT_EQ(table.end(), it);
		EXPECT_EQ(table.cend(), cit);
		for(int i = 0; i < size_i; ++i) {
			EXPECT_EQ(key_histogram[i], 4);
		}
		EXPECT_GT((table.load_factor() * 4.0f), static_cast<float>(table.load_max()));
		for(int i = 0; i < (size_i/2); ++i) {
			EXPECT_EQ(table.erase(hxtest_object(i)), 2);
		}
		for(int i = (size_i/2); i < size_i; ++i) {
			hxptr<hxtest_object_node> ti = table.extract(hxtest_object(i));
			EXPECT_EQ(ti->hash_key().value(), i);
		}
		for(int i = 0; i < (size_i/2); ++i) {
			EXPECT_EQ(table.release_key(hxtest_object(i)), 0);
			EXPECT_EQ(table.find(hxtest_object(i)), table.end());
		}
		for(int i = (size_i/2); i < size_i; ++i) {
			const table_t::iterator ti = table.find(hxtest_object(i));
			EXPECT_EQ(ti->hash_key().value(), i);
			EXPECT_EQ(table.find(hxtest_object(i), ti), table.end());
			EXPECT_EQ(table.count(hxtest_object(i)), 1);
		}
		it = table.begin();
		cit = table.begin();
		for(int i = 0; i < (size_i/2); ++i) {
			++it;
			cit++;
		}
		EXPECT_EQ(table.end(), it);
		EXPECT_EQ(table.cend(), cit);
		hxtest_gdb_break_hxhash_table();
	}
#if (HX_HARDENING_MODE) == HX_HARDENING_MODE_NONE
	EXPECT_TRUE(check_stats(1365, 0, 1053, 312, 0, 0, 0, 1801, 0, 0));
#else
	EXPECT_TRUE(check_stats(1365, 0, 1053, 312, 0, 0, 0, 2419, 0, 0));
#endif
}

TEST_F(hxhash_table_test_f, strings) {
	static const char* const colors[] = {
		"Red","Orange","Yellow",
		"Green","Cyan","Blue",
		"Indigo","Violet" };
	const hxsize_t sz = hxsize(colors);
	{
		const hxslab_allocator_scope stack_scope(hxslab_allocator_stack_0);
		using table_t = hxhash_table<hxtest_string, hxdefault_delete, 4, 0>;
		table_t table;
		for(hxsize_t i = sz; i-- != 0;) {
			hxtest_string* n = hxnew<hxtest_string>(colors[i], static_cast<int32_t>(i));
			EXPECT_EQ(&*table.insert(hxptr<hxtest_string>(n)), n);
			EXPECT_STREQ(n->hash_key(), colors[i]);
		}
		EXPECT_NE(table.find("Cyan"), table.end());
		EXPECT_EQ(table.find("Pink"), table.end());
	}
	EXPECT_TRUE(check_stats(static_cast<int>(sz),
		0, static_cast<int>(sz), 0, 0, 0, 0, 0, 0, 0));
}

TEST(hxhash_table_test, string_literal_nodes) {
	static const char* const literals[] = {
		"Crimson", "Teal", "Magenta", "Gold"
	};
	using table_t = hxhash_table<hxtest_string_literal, hxdefault_delete, 4, 0>;
	table_t table;
	for(hxsize_t i = 0; i < hxsize(literals); ++i) {
		hxtest_string_literal* n = hxnew<hxtest_string_literal>(literals[i]);
		EXPECT_EQ(&*table.insert(hxptr<hxtest_string_literal>(n)), n);
		EXPECT_EQ(n->hash_key(), literals[i]);
		EXPECT_EQ(n->hash_value(), (hxkey_hash(literals[i])));
	}
	EXPECT_EQ(table.size(), hxsize(literals));
	EXPECT_NE(table.find("Crimson"), table.end());
	EXPECT_EQ(table.find("Cerulean"), table.end());
}

TEST(hxhash_table_set_node_test, operator_equal_and_less) {
	const hxtest_set_node_t a(31), b(31), c(32);
	EXPECT_TRUE(a == b);
	EXPECT_FALSE(a == c);
	EXPECT_FALSE(a < b);
	EXPECT_FALSE(b < a);
	EXPECT_TRUE(a < c);
	EXPECT_FALSE(c < a);
}

#if HX_CPLUSPLUS >= 202002L
TEST(hxhash_table_map_node_test, hash_next_type) {
	using node_t = hxhash_table_map_node<int32_t, int32_t>;
	using table_t = hxhash_table<node_t, hxdo_not_delete, 4, hxtrait_multi>;
	// d shares 1's bucket at table_size_bits 4.
	node_t nodes[] = { node_t(1, 10), node_t(4, 40), node_t(1, 20), node_t(2, 30) };
	table_t table;
	table.add_range(hxmake_range(nodes, nodes + hxsize(nodes)));
	EXPECT_EQ(table.size(), 4);
	const table_t::iterator first = table.find(1);
	ASSERT_NE(first, table.end());
	const table_t::iterator second = table.find(1, first);
	ASSERT_NE(second, table.end());
	EXPECT_NE(first, second);
	EXPECT_EQ(table.find(1, second), table.end());
	hxsize_t count = 0;
	for(table_t::const_iterator it = table.begin(); it != table.end(); ++it) {
		++count;
	}
	EXPECT_EQ(count, 4);
	table.release_all();
}
#endif // HX_CPLUSPLUS >= 202002L
struct hxtest_map_node_t : hxhash_table_map_node<int32_t, int32_t> {
	explicit hxtest_map_node_t(int32_t k, int32_t v)
		: hxhash_table_map_node<int32_t, int32_t>(k, v) { }
};

#if HX_CPLUSPLUS >= 202002L
TEST(hxhash_table_map_node_test, subclass_hash_next_type) {
	using table_t = hxhash_table<hxtest_map_node_t, hxdo_not_delete, 4, 0>;
	hxtest_map_node_t nodes[] = {
		hxtest_map_node_t(1, 10), hxtest_map_node_t(2, 20), hxtest_map_node_t(3, 30) };
	table_t table;
	table.add_range(hxmake_range(nodes, nodes + hxsize(nodes)));
	EXPECT_EQ(table.size(), 3);
	const table_t::iterator found = table.find(2);
	ASSERT_NE(found, table.end());
	EXPECT_EQ(found->value(), 20);
	EXPECT_EQ(table.release_key(2), 1);
	EXPECT_EQ(table.size(), 2);
	EXPECT_EQ(table.find(2), table.end());
	table.release_all();
}

TEST(hxhash_table_map_node_test, add_range_empty_range_leaves_table_empty) {
	using node_t = hxhash_table_map_node<int32_t, int32_t>;
	using table_t = hxhash_table<node_t, hxdo_not_delete, 4, 0>;
	node_t* const empty_begin = hxnull;
	table_t table;
	table.add_range(hxmake_range(empty_begin, empty_begin));
	EXPECT_EQ(table.size(), 0);
	EXPECT_EQ(table.begin(), table.end());
}

TEST(hxhash_table_map_node_test, add_range_appends_to_nonempty_table) {
	using node_t = hxhash_table_map_node<int32_t, int32_t>;
	using table_t = hxhash_table<node_t, hxdo_not_delete, 4, 0>;
	node_t first(31, 310);
	node_t more[] = { node_t(32, 320), node_t(33, 330) };
	table_t table;
	table.insert(&first);
	table.add_range(hxmake_range(more, more + hxsize(more)));
	EXPECT_EQ(table.size(), 3);
	EXPECT_NE(table.find(31), table.end());
	EXPECT_NE(table.find(32), table.end());
	EXPECT_NE(table.find(33), table.end());
	table.release_all();
}
#endif // HX_CPLUSPLUS >= 202002L

TEST(hxhash_table_map_node_test, operator_equal_and_less) {
	using node_t = hxhash_table_map_node<int32_t, int32_t>;
	const node_t same_key_same_value_a(31, 100), same_key_same_value_b(31, 100);
	const node_t same_key_diff_value(31, 101);
	const node_t diff_key_same_value(32, 100);
	EXPECT_TRUE(same_key_same_value_a == same_key_same_value_b);
	EXPECT_FALSE(same_key_same_value_a == same_key_diff_value);
	EXPECT_FALSE(same_key_same_value_a == diff_key_same_value);
	EXPECT_FALSE(same_key_same_value_a < same_key_same_value_b);
	EXPECT_FALSE(same_key_same_value_b < same_key_same_value_a);
	EXPECT_TRUE(same_key_same_value_a < same_key_diff_value);
	EXPECT_FALSE(same_key_diff_value < same_key_same_value_a);
	EXPECT_TRUE(same_key_same_value_a < diff_key_same_value);
	EXPECT_FALSE(diff_key_same_value < same_key_same_value_a);
}

TEST_F(hxhash_table_test_f, iterator_traverses_all_nodes_exactly_once) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, hxtrait_multi>;
	table_t table;
	static const int keys[] = { 1, 2, 3, 4, 5 };
	static const int n = static_cast<int>(sizeof(keys) / sizeof(keys[0]));
	for(int i = 0; i < n; ++i) {
		table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(keys[i], keys[i])));
	}
	EXPECT_EQ(table.size(), static_cast<hxsize_t>(n));
	int visited = 0;
	for(table_t::const_iterator it = table.cbegin(); it != table.cend(); ++it) {
		++visited;
	}
	EXPECT_EQ(visited, n);
	EXPECT_TRUE(check_stats(n, 0, n, 0, 0, 0, 0, 0, 0, 0));
}

TEST(hxhash_table_test, iterator_begin_equals_end_on_empty_table) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
	table_t table;
	EXPECT_EQ(table.begin(), table.end());
	EXPECT_EQ(table.cbegin(), table.cend());
}

TEST_F(hxhash_table_test_f, default_constructed_iterators_equal_end) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
	table_t table;
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(10, 10)));
	const table_t::const_iterator cd;
	EXPECT_EQ(cd, table.cend());
	EXPECT_FALSE(cd == table.cbegin());
	const table_t::iterator d;
	EXPECT_EQ(d, table.end());
	EXPECT_FALSE(d == table.begin());
	EXPECT_TRUE(check_stats(1, 0, 1, 0, 0, 0, 0, 0, 0, 0));
}

TEST(hxhash_table_test, bucket_count_matches_table_size_bits) {
	using table_static_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
	const table_static_t static_table;
	EXPECT_EQ(static_table.bucket_count(), 16);
	using table_dynamic_t = hxhash_table<hxtest_integer, hxdefault_delete, hxallocator_dynamic_capacity, 0>;
	table_dynamic_t dynamic_table;
	dynamic_table.set_size_bits(5);
	EXPECT_EQ(dynamic_table.bucket_count(), 32);
}

TEST_F(hxhash_table_test_f, const_iterator_post_increment_returns_prior) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
	table_t table;
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(10, 10)));
	table_t::const_iterator it = table.cbegin();
	const table_t::const_iterator prior = it++;
	EXPECT_EQ(prior, table.cbegin());
	EXPECT_EQ(it, table.cend());
	EXPECT_TRUE(check_stats(1, 0, 1, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, emplace_constructs_and_inserts_node) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, hxtrait_multi>;
	table_t table;
	const table_t::iterator a = table.emplace(7, 7);
	EXPECT_EQ(a->hash_key(), 7);
	EXPECT_EQ(table.size(), 1);
	const table_t::iterator b = table.emplace(7, 7);
	EXPECT_NE(a, b);
	EXPECT_EQ(b->hash_key(), 7);
	EXPECT_EQ(table.size(), 2);
	EXPECT_EQ(table.count(7), 2);
	EXPECT_TRUE(check_stats(2, 0, 2, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, try_emplace_inserts_then_returns_existing) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
	table_t table;
	const table_t::iterator a = table.try_emplace(7, 7, 7);
	EXPECT_EQ(a->hash_key(), 7);
	EXPECT_EQ(table.size(), 1);
	const table_t::iterator b = table.try_emplace(7, 7, 7);
	EXPECT_EQ(a, b);
	EXPECT_EQ(table.size(), 1);
	EXPECT_TRUE(check_stats(1, 0, 1, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, count_returns_zero_for_absent_key) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
	table_t table;
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(10, 10)));
	EXPECT_EQ(table.count(10), 1);
	EXPECT_EQ(table.count(99), 0);
	EXPECT_TRUE(check_stats(1, 0, 1, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, count_multi_two_same_key) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, hxtrait_multi>;
	table_t table;
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(7, 7)));
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(7, 7)));
	EXPECT_EQ(table.count(7), 2);
	EXPECT_TRUE(check_stats(2, 0, 2, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, find_second_duplicate_via_previous) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, hxtrait_multi>;
	table_t table;
	hxtest_integer* n1 = hxnew<hxtest_integer>(34, 34);
	// Shares 34's bucket at table_size_bits 4.
	hxtest_integer* const colliding = hxnew<hxtest_integer>(3, 3);
	hxtest_integer* n2 = hxnew<hxtest_integer>(34, 34);
	table.insert(hxptr<hxtest_integer>(n1));
	table.insert(hxptr<hxtest_integer>(colliding));
	table.insert(hxptr<hxtest_integer>(n2));
	EXPECT_EQ(table.size(), 3u);
	const table_t::iterator first = table.find(34);
	ASSERT_NE(first, table.end());
	const table_t::iterator second = table.find(34, first);
	ASSERT_NE(second, table.end());
	EXPECT_NE(second, first);
	EXPECT_EQ(table.find(34, second), table.end());

	using single_bucket_table_t = hxhash_table<hxtest_integer, hxdefault_delete, 1, hxtrait_multi>;
	single_bucket_table_t single_bucket_table;
	hxtest_integer* tail_match = hxnew<hxtest_integer>(0, 0);
	hxtest_integer* middle_other = hxnew<hxtest_integer>(2, 2);
	hxtest_integer* head_match = hxnew<hxtest_integer>(0, 0);
	single_bucket_table.insert(hxptr<hxtest_integer>(tail_match));
	single_bucket_table.insert(hxptr<hxtest_integer>(middle_other));
	single_bucket_table.insert(hxptr<hxtest_integer>(head_match));
	const single_bucket_table_t::iterator found_head = single_bucket_table.find(0);
	EXPECT_EQ(&*found_head, head_match);
	const single_bucket_table_t::iterator found_tail = single_bucket_table.find(0, found_head);
	EXPECT_EQ(&*found_tail, tail_match);
	EXPECT_EQ(single_bucket_table.find(0, found_tail), single_bucket_table.end());
	const single_bucket_table_t::iterator found_middle = single_bucket_table.find(2);
	EXPECT_EQ(single_bucket_table.find(2, found_middle), single_bucket_table.end());
	EXPECT_TRUE(check_stats(6, 0, 6, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, find_with_previous_skips_colliding_key_in_single_table) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
	table_t table;
	// Shares 77's bucket at table_size_bits 4.
	hxtest_integer* const node_17 = hxnew<hxtest_integer>(17, 17);
	hxtest_integer* const node_77 = hxnew<hxtest_integer>(77, 77);
	table.insert(hxptr<hxtest_integer>(node_17));
	table.insert(hxptr<hxtest_integer>(node_77));
	const table_t::iterator found = table.find(77);
	EXPECT_EQ(table.find(77, found), table.end());
	EXPECT_TRUE(check_stats(2, 0, 2, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, find_absent_key_in_nonempty_bucket_chain) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, hxallocator_dynamic_capacity, hxtrait_multi>;
	table_t table;
	table.set_size_bits(1);
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(1, 1)));
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(3, 3)));
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(5, 5)));
	EXPECT_EQ(table.find(99), table.end());
	EXPECT_NE(table.find(1), table.end());
	EXPECT_TRUE(check_stats(3, 0, 3, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, insert_two_keys_same_bucket_both_findable) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, hxallocator_dynamic_capacity, hxtrait_multi>;
	table_t table;
	table.set_size_bits(1);
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(1, 1)));
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(3, 3)));
	EXPECT_NE(table.find(1), table.end());
	EXPECT_NE(table.find(3), table.end());
	EXPECT_EQ(table.size(), 2u);
	EXPECT_TRUE(check_stats(2, 0, 2, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, erase_head_node_updates_size) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
	table_t table;
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(5, 5)));
	hxtest_integer* const node_6 = hxnew<hxtest_integer>(6, 6);
	table.insert(hxptr<hxtest_integer>(node_6));
	EXPECT_EQ(table.erase(6, hxdo_not_delete()), 1u);
	EXPECT_EQ(table.size(), 1u);
	EXPECT_EQ(table.find(6), table.end());
	EXPECT_NE(table.find(5), table.end());
	hxdelete(node_6);
	EXPECT_EQ(table.erase(5), 1u);
	EXPECT_EQ(table.size(), 0u);
	EXPECT_EQ(table.find(5), table.end());

	using single_bucket_table_t = hxhash_table<hxtest_integer, hxdefault_delete, 1, 0>;
	single_bucket_table_t single_bucket_table;
	single_bucket_table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(1, 1)));
	hxtest_integer* const node_2 = hxnew<hxtest_integer>(2, 2);
	single_bucket_table.insert(hxptr<hxtest_integer>(node_2));
	EXPECT_EQ(single_bucket_table.erase(2, hxdo_not_delete()), 1u);
	EXPECT_EQ(single_bucket_table.size(), 1u);
	EXPECT_EQ(single_bucket_table.find(2), single_bucket_table.end());
	EXPECT_NE(single_bucket_table.find(1), single_bucket_table.end());
	hxdelete(node_2);

	using fn_deleter_table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
	fn_deleter_table_t fn_deleter_table;
	fn_deleter_table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(9, 9)));
	EXPECT_EQ(fn_deleter_table.erase(9, &hxdelete<hxtest_integer>), 1u);
	EXPECT_EQ(fn_deleter_table.size(), 0u);
	EXPECT_EQ(fn_deleter_table.find(9), fn_deleter_table.end());
	EXPECT_TRUE(check_stats(5, 0, 5, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, erase_interior_node_in_bucket_chain) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, hxallocator_dynamic_capacity, hxtrait_multi>;
	table_t table;
	table.set_size_bits(1);
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(1, 1)));
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(3, 3)));
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(5, 5)));
	const hxsize_t erased = table.erase(3);
	EXPECT_EQ(erased, 1u);
	EXPECT_EQ(table.size(), 2u);
	EXPECT_EQ(table.find(3), table.end());
	EXPECT_NE(table.find(1), table.end());
	EXPECT_NE(table.find(5), table.end());

	using single_bucket_table_t = hxhash_table<hxtest_integer, hxdefault_delete, 1, hxtrait_multi>;
	{
		single_bucket_table_t all_matching_table;
		hxtest_integer* const all_first = hxnew<hxtest_integer>(0, 0);
		hxtest_integer* const all_second = hxnew<hxtest_integer>(0, 0);
		hxtest_integer* const all_third = hxnew<hxtest_integer>(0, 0);
		all_matching_table.insert(hxptr<hxtest_integer>(all_first));
		all_matching_table.insert(hxptr<hxtest_integer>(all_second));
		all_matching_table.insert(hxptr<hxtest_integer>(all_third));
		EXPECT_EQ(all_matching_table.erase(0, &hxdelete<hxtest_integer>), 3u);
		EXPECT_EQ(all_matching_table.size(), 0u);
		EXPECT_EQ(all_matching_table.find(0), all_matching_table.end());
	}
	{
		single_bucket_table_t all_matching_no_delete_table;
		hxtest_integer* const nd_first = hxnew<hxtest_integer>(0, 0);
		hxtest_integer* const nd_second = hxnew<hxtest_integer>(0, 0);
		all_matching_no_delete_table.insert(hxptr<hxtest_integer>(nd_first));
		all_matching_no_delete_table.insert(hxptr<hxtest_integer>(nd_second));
		EXPECT_EQ(all_matching_no_delete_table.erase(0, hxdo_not_delete()), 2u);
		EXPECT_EQ(all_matching_no_delete_table.size(), 0u);
		hxdelete(nd_first);
		hxdelete(nd_second);
	}
	{
		single_bucket_table_t suffix_matching_table;
		hxtest_integer* const suffix_first = hxnew<hxtest_integer>(0, 0);
		hxtest_integer* const suffix_second = hxnew<hxtest_integer>(0, 0);
		suffix_matching_table.insert(hxptr<hxtest_integer>(suffix_first));
		suffix_matching_table.insert(hxptr<hxtest_integer>(suffix_second));
		suffix_matching_table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(2, 2)));
		EXPECT_EQ(suffix_matching_table.erase(0, &hxdelete<hxtest_integer>), 2u);
		EXPECT_EQ(suffix_matching_table.size(), 1u);
		EXPECT_EQ(suffix_matching_table.find(0), suffix_matching_table.end());
		EXPECT_NE(suffix_matching_table.find(2), suffix_matching_table.end());
	}
	{
		single_bucket_table_t prefix_run_then_mismatch_table;
		prefix_run_then_mismatch_table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(2, 2)));
		hxtest_integer* const prefix_second = hxnew<hxtest_integer>(0, 0);
		hxtest_integer* const prefix_first = hxnew<hxtest_integer>(0, 0);
		prefix_run_then_mismatch_table.insert(hxptr<hxtest_integer>(prefix_second));
		prefix_run_then_mismatch_table.insert(hxptr<hxtest_integer>(prefix_first));
		EXPECT_EQ(prefix_run_then_mismatch_table.erase(0, &hxdelete<hxtest_integer>), 2u);
		EXPECT_EQ(prefix_run_then_mismatch_table.size(), 1u);
		EXPECT_EQ(prefix_run_then_mismatch_table.find(0), prefix_run_then_mismatch_table.end());
		EXPECT_NE(prefix_run_then_mismatch_table.find(2), prefix_run_then_mismatch_table.end());
	}
	{
		single_bucket_table_t interior_matching_table;
		hxtest_integer* const interior_head = hxnew<hxtest_integer>(2, 2);
		hxtest_integer* const interior_match = hxnew<hxtest_integer>(0, 0);
		hxtest_integer* const interior_tail = hxnew<hxtest_integer>(2, 2);
		interior_matching_table.insert(hxptr<hxtest_integer>(interior_tail));
		interior_matching_table.insert(hxptr<hxtest_integer>(interior_match));
		interior_matching_table.insert(hxptr<hxtest_integer>(interior_head));
		EXPECT_EQ(interior_matching_table.erase(0, hxdo_not_delete()), 1u);
		EXPECT_EQ(interior_matching_table.size(), 2u);
		EXPECT_EQ(interior_matching_table.find(0), interior_matching_table.end());
		EXPECT_EQ(interior_matching_table.count(2), 2);
		hxdelete(interior_match);
		interior_matching_table.release_all();
		hxdelete(interior_head);
		hxdelete(interior_tail);
	}
	EXPECT_TRUE(check_stats(17, 0, 17, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, erase_returns_zero_for_absent_key) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
	table_t table;
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(10, 10)));
	EXPECT_EQ(table.erase(99), 0u);
	EXPECT_EQ(table.size(), 1u);
	EXPECT_TRUE(check_stats(1, 0, 1, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, extract_head_node_removes_from_table) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 4, 0>;
	table_t table;
	hxtest_integer* node = hxnew<hxtest_integer>(55, 55);
	table.insert(hxptr<hxtest_integer>(node));
	const hxptr<hxtest_integer> extracted = table.extract(55);
	EXPECT_EQ(extracted.get(), node);
	EXPECT_EQ(table.find(55), table.end());
	EXPECT_EQ(table.size(), 0u);
	EXPECT_TRUE(check_stats(1, 0, 1, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, extract_interior_node_keeps_others) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, hxallocator_dynamic_capacity, hxtrait_multi>;
	table_t table;
	table.set_size_bits(1);
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(1, 1)));
	hxtest_integer* mid = hxnew<hxtest_integer>(3, 3);
	table.insert(hxptr<hxtest_integer>(mid));
	table.insert(hxptr<hxtest_integer>(hxnew<hxtest_integer>(5, 5)));
	const hxptr<hxtest_integer> extracted = table.extract(3);
	EXPECT_EQ(extracted.get(), mid);
	EXPECT_EQ(table.size(), 2u);
	EXPECT_NE(table.find(1), table.end());
	EXPECT_NE(table.find(5), table.end());
	EXPECT_EQ(table.find(3), table.end());
	EXPECT_TRUE(check_stats(3, 0, 3, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, erase_iterator_head_node_returns_next) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 1, hxtrait_multi>;
	table_t table;
	hxtest_integer* const node_2 = hxnew<hxtest_integer>(2, 2);
	table.insert(hxptr<hxtest_integer>(node_2));
	hxtest_integer* const node_0 = hxnew<hxtest_integer>(0, 0);
	table.insert(hxptr<hxtest_integer>(node_0));
	const table_t::iterator it = table.begin();
	EXPECT_EQ(&*it, node_0);
	const table_t::iterator next = table.erase(it);
	EXPECT_EQ(&*next, node_2);
	EXPECT_EQ(table.size(), 1u);
	EXPECT_EQ(table.find(0), table.end());
	EXPECT_NE(table.find(2), table.end());
	EXPECT_TRUE(check_stats(2, 0, 2, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, erase_iterator_interior_node_returns_next) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 1, hxtrait_multi>;
	table_t table;
	hxtest_integer* const node_10 = hxnew<hxtest_integer>(10, 10);
	table.insert(hxptr<hxtest_integer>(node_10));
	hxtest_integer* const node_9 = hxnew<hxtest_integer>(9, 9);
	table.insert(hxptr<hxtest_integer>(node_9));
	hxtest_integer* const node_2 = hxnew<hxtest_integer>(2, 2);
	table.insert(hxptr<hxtest_integer>(node_2));
	hxtest_integer* const node_0 = hxnew<hxtest_integer>(0, 0);
	table.insert(hxptr<hxtest_integer>(node_0));
	table_t::iterator it = table.begin();
	while(&*it != node_9) {
		++it;
	}
	const table_t::iterator next = table.erase(it);
	EXPECT_EQ(&*next, node_10);
	EXPECT_EQ(table.size(), 3u);
	EXPECT_EQ(table.find(9), table.end());
	EXPECT_TRUE(check_stats(4, 0, 4, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, erase_iterator_tail_node_returns_end) {
	using table_t = hxhash_table<hxtest_integer, hxdefault_delete, 1, hxtrait_multi>;
	table_t table;
	hxtest_integer* const node_2 = hxnew<hxtest_integer>(2, 2);
	table.insert(hxptr<hxtest_integer>(node_2));
	hxtest_integer* const node_0 = hxnew<hxtest_integer>(0, 0);
	table.insert(hxptr<hxtest_integer>(node_0));
	table_t::iterator it = table.begin();
	while(&*it != node_2) {
		++it;
	}
	const table_t::iterator next = table.erase(it);
	EXPECT_EQ(next, table.end());
	EXPECT_EQ(table.size(), 1u);
	EXPECT_EQ(table.find(2), table.end());
	EXPECT_NE(table.find(0), table.end());
	EXPECT_TRUE(check_stats(2, 0, 2, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, erase_iterator_do_not_delete_skips_deleter) {
	using table_t = hxhash_table<hxtest_integer, hxdo_not_delete, 4, 0>;
	table_t table;
	hxtest_integer node_31(31, 31);
	table.insert(hxptr<hxtest_integer, hxdo_not_delete>(&node_31));
	const table_t::iterator it = table.begin();
	const table_t::iterator next = table.erase(it);
	EXPECT_EQ(next, table.end());
	EXPECT_EQ(table.size(), 0u);
	EXPECT_TRUE(check_stats(1, 0, 1, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, replace_no_match_inserts_new_node) {
	using table_t = hxhash_table<hxtest_integer, hxdo_not_delete, 4, 0>;
	table_t table;
	hxtest_integer* const node_1 = hxnew<hxtest_integer>(1, 1);
	const hxptr<hxtest_integer, hxdo_not_delete> replaced = table.replace(node_1);
	EXPECT_EQ(replaced.get(), hxnil);
	EXPECT_EQ(table.size(), 1u);
	EXPECT_EQ(&*table.find(1), node_1);
	hxdelete(node_1);
	EXPECT_TRUE(check_stats(1, 0, 1, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, replace_head_match_keeps_size_and_swaps_node) {
	using table_t = hxhash_table<hxtest_integer, hxdo_not_delete, 1, 0>;
	table_t table;
	hxtest_integer* const node_2 = hxnew<hxtest_integer>(2, 2);
	table.insert(hxptr<hxtest_integer>(node_2));
	hxtest_integer* const node_0a = hxnew<hxtest_integer>(0, 0);
	table.insert(hxptr<hxtest_integer>(node_0a));
	hxtest_integer* const node_0b = hxnew<hxtest_integer>(0, 0);
	const hxptr<hxtest_integer, hxdo_not_delete> replaced = table.replace(node_0b);
	EXPECT_EQ(replaced.get(), node_0a);
	EXPECT_EQ(table.size(), 2u);
	EXPECT_EQ(&*table.find(0), node_0b);
	EXPECT_NE(table.find(2), table.end());
	hxdelete(node_0a);
	hxdelete(node_0b);
	hxdelete(node_2);
	EXPECT_TRUE(check_stats(3, 0, 3, 0, 0, 0, 0, 0, 0, 0));
}

TEST_F(hxhash_table_test_f, replace_interior_match_keeps_size_and_swaps_node) {
	using table_t = hxhash_table<hxtest_integer, hxdo_not_delete, 1, 0>;
	table_t table;
	hxtest_integer* const node_9 = hxnew<hxtest_integer>(9, 9);
	table.insert(hxptr<hxtest_integer>(node_9));
	hxtest_integer* const node_0a = hxnew<hxtest_integer>(0, 0);
	table.insert(hxptr<hxtest_integer>(node_0a));
	hxtest_integer* const node_2 = hxnew<hxtest_integer>(2, 2);
	table.insert(hxptr<hxtest_integer>(node_2));
	hxtest_integer* const node_0b = hxnew<hxtest_integer>(0, 0);
	const hxptr<hxtest_integer, hxdo_not_delete> replaced = table.replace(node_0b);
	EXPECT_EQ(replaced.get(), node_0a);
	EXPECT_EQ(table.size(), 3u);
	EXPECT_EQ(&*table.find(0), node_0b);
	EXPECT_NE(table.find(2), table.end());
	EXPECT_NE(table.find(9), table.end());
	hxdelete(node_0a);
	hxdelete(node_0b);
	hxdelete(node_2);
	hxdelete(node_9);
	EXPECT_TRUE(check_stats(4, 0, 4, 0, 0, 0, 0, 0, 0, 0));
}

TEST(hxhash_table_test, gdb_printer_edge_cases) {
	using dynamic_set_table_t = hxhash_table<hxtest_set_node_t, hxdo_not_delete, hxallocator_dynamic_capacity, 0>;
	const dynamic_set_table_t a;
	EXPECT_EQ(a.bucket_count(), 0);

	hxtest_set_node_t node_b(31);
	dynamic_set_table_t b;
	b.set_size_bits(1);
	b.insert(&node_b);
	EXPECT_EQ(b.size(), 1u);

	using map_table_t = hxhash_table<hxtest_map_node_t, hxdo_not_delete, 1, 0>;
	hxtest_map_node_t node_c(31, 32);
	map_table_t c;
	c.insert(&node_c);
	EXPECT_EQ(c.size(), 1u);

	hxtest_gdb_break_hxhash_table_edge_cases();

	b.release_all();
	c.release_all();
}

#if HX_CPLUSPLUS >= 202002L
TEST(hxhash_table_node_integer_test, conforms_to_forward_iterator_api) {
	using table_t = hxhash_table<hxhash_table_node_integer<int32_t>, hxdo_not_delete, 4, 0>;
	hxhash_table_node_integer<int32_t> nodes[] = { 1, 2, 3 };
	table_t table;
	table.add_range(hxmake_range(nodes, nodes + hxsize(nodes)));
	EXPECT_TRUE(hxtest_check_forward_iterator_api(table.begin(), table.end()));
	const table_t& const_table = table;
	EXPECT_TRUE(hxtest_check_forward_iterator_api(const_table.begin(), const_table.end()));
	table.release_all();
}
#endif // HX_CPLUSPLUS >= 202002L
